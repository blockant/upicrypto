import { Button, Chip, Divider, Drawer, FormControl, FormControlLabel, FormLabel, Grid, InputLabel, List, ListItem, ListItemButton, ListItemIcon, ListItemText, MenuItem, Modal, Radio, RadioGroup, Select, Slide, TextField, TextFieldProps, Tooltip } from '@mui/material';
...
const Payment = (props: PaymentProps) => {
    ...
    const transactionCharges: number = (props.paymentFormProps.transactionAmount > 0) ? props.paymentFormProps.transactionAmount : 1.76;
    ...
    useEffect(() => {
        ...
        if(!props.paymentFormProps.transactionCurrency) {
            ...
            setTransactionCurrency(currencies.filter(curr => (curr.abbreviation === props.paymentFormProps.transactionCurrency))[0])
        }
    }, []);
    ...
    const paymentValidation = (payee: string, amount: number, selectedCurrency: CurrencyAmount, transactionCharges: number, selectedTransactionCurrency: CurrencyAmount) => {
        let payeeError = payeeValidation(payee);
        let amountError = amountValidation(amount, selectedCurrency);
        let transactionChargesError = amountValidation(transactionCharges, selectedTransactionCurrency);
        setErrorState({
            ...errorState,
    ...
    const onPaymentClick = (event: any) => {
        ...
        if(!paymentValidation(payee, amount, currency, transactionCharges, transactionCurrency)) {
            return;
        }
    ...
    const renderBalanceCheck = (curr: CurrencyAmount) => {
        if(curr.abbreviation !== currency.abbreviation && curr.abbreviation !== transactionCurrency.abbreviation){
            return <></>;
        }

        if(currency.abbreviation === transactionCurrency.abbreviation) {
            if(amount+transactionCharges > curr.amount) {
                return <Chip label='Insufficient balance' color='error' variant='outlined'/>
            }
            return <Tooltip title='Balance is sufficient for transaction.'>
                <CheckIcon color='success'/>
            </Tooltip>
        }
        
        if(curr.abbreviation === currency.abbreviation && amount > curr.amount) {
            return <Chip label='Insufficient balance for amount' color='error' variant='outlined'/>
        }
        if(curr.abbreviation === transactionCurrency.abbreviation && transactionCharges > curr.amount) {
            return <Chip label='Insufficient balance for txn charges' color='error' variant='outlined'/>
        }
            ...
            case 'IDLE': return <>
                <div className="payment-details-container">
                        ...
                        <List disablePadding>
                            {
                                availableCurrency.map((curr) => <>
                                    <ListItem
                                        secondaryAction={renderBalanceCheck(curr)}
                                    >
                    ...
                    <div className="payment-button-container fullWidth">
                        <Button variant="contained" onClick={onPaymentClick}>Pay</Button>
                    </div>
                </>
            case 'LOADING': return <PaymentLoading />
            case 'PAYMENT_SUCCESS': return <PaymentStatus status={paymentStatus}/>
            case 'PAYMENT_FAILED': return <PaymentStatus status={paymentStatus}/>
            default: return <></>
        }
    }
    ...
    return <div className="payment-container">
        <div style={{display: 'flex', justifyContent: 'space-between'}}>
            <h1>Payment</h1>
            <CloseIcon onClick={() => props.closePayment(false, 'CLOSE_PAYMENT')}></CloseIcon>
        </div> 
        {
